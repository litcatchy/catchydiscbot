...
@commands.command()
    async def untimeout(self, ctx, member: discord.Member = None):
        ...
        if member == ctx.guild.owner:
            embed = discord.Embed(
                title="❌ Cannot UnTimeout Server Owner",
        ...
        if not ctx.guild.me.guild_permissions.moderate_members:
            embed = discord.Embed(
                title="❌ Missing Bot Permissions",
                description="I don’t have permission to modify timeouts.",
                color=discord.Color.red()
            )
            await ctx.send(embed=embed)
            return
        ...
        # Success message
        embed = discord.Embed(
            title="✅ User UnTimeouted",
            description=f"{member.mention}'s timeout has been removed. <:currencypaw:1346100210899619901>",
            color=discord.Color.green()
        )
        embed.set_footer(text=f"UnTimeouted by {ctx.author}", icon_url=ctx.author.display_avatar.url)
        await ctx.send(embed=embed)
      async def setup(bot):
    await bot.add_cog(Moderation(bot))
